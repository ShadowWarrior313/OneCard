...
/**
 * Check 12 — Hub user identity must not be derived from AUTH_SESSION_SECRET.
 *
 ...
 *
 * Numbered 12 so it does not collide with open hardening checks 9–11
 * (hub auth/webhook, mock isolation, Stripe test-mode).
 */
function checkHubUserIdentity(files: string[]): CheckResult {
  ...
  const identityFile = files.find((f) => /apps\/web\/src\/data\/hubUserIdentity\.(ts|js)$/.test(f));
  ...
  if (!sessionFile || !storeFile) {
    ...
    return { id: "12", title: "Hub user identity independent of session secret", findings, notes };
  }
  ...
  const identitySource = identityFile ? read(identityFile) : "";
  const combined = `${storeSource}\n${identitySource}`;
  const upsertsByEmail =
    /upsertHubUserByEmail/.test(storeSource) ||
    /user\.email\s*===\s*email/.test(combined) ||
    /find\(\s*\(user\)\s*=>\s*user\.email\s*===\s*/.test(combined);

  if (!upsertsByEmail) {
    findings.push({
      severity: "FAIL",
      file: identityFile ?? storeFile,
