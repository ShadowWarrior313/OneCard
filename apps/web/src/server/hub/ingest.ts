 ...
 *
 * Pass `{ throwOnFailure: true }` from webhook handlers so providers can retry
 * when sync fails after verification.
 */
export async function syncLinkedItem(
  ...
  try {
    ...
    await mutateHubStore((nextStore) => {
      ...
      if (!mutable) return;
      // Another sync may have advanced the cursor/token while we were in flight.
