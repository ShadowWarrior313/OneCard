...
export function PhoneWalletFold({
  ...
  const hasAddable = CARD_CATALOG.some((c) => !hasCard(c.cardId));
      ...
      <div ref={ref} className="space-y-4 pb-4">
        ...
        {hasAddable && (
          <div className="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm">
            ...
            <div className="mt-3">
              <CompactCardAdder hasCard={hasCard} onAdd={onToggleCard} compact />
            </div>
          </div>
        )}
      </div>
    ...
    <div className="rounded-xl border border-zinc-200 bg-white p-3 shadow-sm">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="whitespace-nowrap text-sm font-semibold text-brand-ink">{card.displayName}</p>
